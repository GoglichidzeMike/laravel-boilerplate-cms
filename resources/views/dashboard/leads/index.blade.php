...
<div class="flex flex-col items-center" >
  ...
  <div class="w-10/12 bg-white p-6 rounded-lg">
    ...
    <div class="flex rounded-sm w-full bg-gray-100 mb-4 shadow">
      ...
      <div class="rounded-sm p-2 rounded-sm border-2 border-r-0 bborder-gray w-2/12">Message</div>
      <div class="rounded-sm p-2 rounded-sm border-2 border-r-0 bborder-gray w-2/12">Event</div>
    ...
    @if($leads->count())
      ...
      <div class="flex rounded-sm w-full bg-gray-10 my-2 shadow-sm hover:shadow-md transition duration-150 ease-in-out">
        <div class="rounded-sm p-2 rounded-sm border-2 border-r-0 border-gray w-2/12 flex justify-between"><a href="{{ route('lead.show', $lead->id) }}">{{ $lead->name }}</a></div>
        ...
        <div class="rounded-sm p-2 rounded-sm border-2 border-r-0 bborder-gray w-2/12">{{ mb_substr($lead->message, 0, 20) }}@if (strlen($lead->message > "20"))...@endif</div>
        <div class="rounded-sm p-2 rounded-sm border-2 border-r-0 bborder-gray w-2/12">{{ mb_substr($lead->referrer, 0, 20) }}</div>
        <div class="rounded-sm p-1 rounded-sm border-2 border-gray">
